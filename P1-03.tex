...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# install the required packages if needed}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Random sample your group\textquotesingle{}s data set from the original LCdata data set}
\NormalTok{LCdata }\OtherTok{\textless{}{-}} \FunctionTok{fread}\NormalTok{(}\StringTok{"LCdata.csv"}\NormalTok{)}
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{03}\NormalTok{)}
...
\FunctionTok{fwrite}\NormalTok{(myLCdata, }\AttributeTok{file =} \StringTok{"myLCdata.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
